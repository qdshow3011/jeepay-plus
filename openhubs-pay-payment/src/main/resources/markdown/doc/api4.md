...
商户业务系统通过转账接口发起转账申请，OpenHubs Pay支付网关将根据请求数据传入到对应的上游接口。

> 接口说明

适用对象：`普通商户` `特约商户`

请求URL：https://pay.OpenHubs Pay.vip/api/transferOrder
...
异步通知地址 | notifyUrl | 否 | String(128) | https://www.openhubs.com/notify.htm | 转账完成后回调该URL,只有传了该值才会发起回调
...
请求URL：https://pay.OpenHubs Pay.vip/api/transfer/query
