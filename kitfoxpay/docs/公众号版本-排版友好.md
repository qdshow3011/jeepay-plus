...
支持多种支付方式（微信、支付宝等，具体取决于 Jeepay 配置），自动处理金额转换、签名验证、参数映射。

**2. 支付结果通知**

接收 Jeepay 的异步通知，自动转换为易支付格式并转发给 NewAPI，支持支付成功、失败、退款等多种通知类型。

**3. 订单查询**

支持单个订单查询、批量订单查询，实时同步订单状态。

**4. 退款功能**

支持全额退款、部分退款，自动处理退款通知转发。
...
提供 Web 管理界面，无需手动编辑配置文件，支持 Jeepay 配置、易支付接口配置、服务器配置等。
...
NewAPI 完全不需要修改代码，只需在 NewAPI 配置中将支付接口地址改为 KitfoxPay 的地址。所有接口参数、签名方式、响应格式完全兼容。

### 2. 平滑升级，解耦维护

NewAPI 的支付逻辑与业务逻辑完全解耦，升级 NewAPI 时，KitfoxPay 独立维护，互不影响。不再需要每次升级都手动合并支付相关代码。

### 3. 开源可控，安全可靠

底层使用开源 Jeepay 支付平台，支持自建部署，数据完全可控。所有代码开源，可审计、可定制。

### 4. 可扩展的适配器架构

采用适配器模式设计，当前支持 Jeepay，后续可以轻松扩展其他支付通道。每个支付通道独立模块，互不干扰。
...
提供 Web 配置管理界面，可视化配置 Jeepay 参数、易支付接口参数，支持配置热更新，无需重启服务。
...
- **Web 框架**：Express.js
- **核心依赖**：express、axios、cors、express-session
...
**`index.js`** - 主入口文件，初始化 Express 服务器，注册路由，处理支付通知和退款通知。

**`epay.js`** - 易支付接口适配器，实现易支付接口标准，参数转换，签名生成与验证，通知格式转换与转发。

**`jeepay/jeepay.js`** - Jeepay 客户端，封装 Jeepay API 调用，签名生成与验证，统一下单、订单查询、退款等接口。

**`admin.js`** - 管理后台 API，配置管理接口，登录认证，配置热更新。

**`public/index.html`** - Web 管理界面，可视化配置管理，配置测试功能。

**`config.js`** - 配置文件，Jeepay 配置、易支付接口配置、服务器配置。
...
看到启动成功提示后，在浏览器中打开 `http://localhost:9219` 访问管理界面。界面展示请参考下方"界面展示"部分。
...
启动服务后，访问 `http://localhost:9219` 即可进入管理界面：
...
希望使用 Jeepay 替代易支付，希望 NewAPI 升级时不受支付代码影响，需要更高的支付安全性和可控性。

**2. 其他使用易支付接口的系统**

任何使用易支付接口标准的系统都可以通过 KitfoxPay 接入 Jeepay，无需修改业务代码，只需更改支付接口地址。

**3. 需要多支付通道聚合的场景**

未来可以扩展支持多个支付通道，通过 KitfoxPay 统一管理，业务系统只需对接一套接口。
...
**提交 Issue**：如果你发现了 Bug 或有功能建议，欢迎提交 Issue。

**提交 Pull Request**：Fork 本仓库，创建特性分支，提交更改，开启 Pull Request。
...
- 感谢 Jeepay 提供优秀的开源支付平台
- 感谢 NewAPI 提供的大模型网关和资产管理系统
- 感谢所有贡献者和使用者的支持
