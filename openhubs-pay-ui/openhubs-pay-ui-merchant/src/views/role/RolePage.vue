<template>
  <page-header-wrapper>
    <a-card>
      <div v-if="$access('ENT_UR_ROLE_SEARCH')" class="table-page-search-wrapper">
        <a-form layout="inline" class="table-head-ground">
          <div class="table-layer">
            <open-hubs-pay-text-up :placeholder="'角色ID'" v-model:value="vdata.searchData.roleId" />
            <open-hubs-pay-text-up :placeholder="'角色名称'" v-model:value="vdata.searchData.roleName" />
      ...
      >
        <template #opRow>
          <a-button v-if="$access('ENT_UR_ROLE_ADD')" type="primary" @click="addFunc">
            新建
          </a-button>
        </template>
