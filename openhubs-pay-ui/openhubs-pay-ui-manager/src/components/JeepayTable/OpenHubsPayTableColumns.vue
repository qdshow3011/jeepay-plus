<script lang="tsx">
import { h } from 'vue'
import JeeMenu from './OpenHubsPayMenu.vue'
import { MenuItem } from 'ant-design-vue'
import OpenHubsPayDrChildren from './OpenHubsPayDrChildren.vue'
// 将 v-if= false的过滤掉
              ...
              {slots
                ...
                .map((item: any) => {
                  if (item.type == 'dr-children' && item.children.length) {
                    ...
                    return (
                      <OpenHubsPayDrChildren title={title}>
                        {item.children
                          .filter((r) => r.props)
                          .map((v) => (
                            <MenuItem>{v}</MenuItem>
                          ))}
                      </OpenHubsPayDrChildren>
                    )
                  } else {
