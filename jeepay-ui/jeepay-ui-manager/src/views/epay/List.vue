  ...
  <page-header-wrapper>
    <a-card>
      <div class="table-page-search-wrapper">
        <a-form layout="inline" v-if="$access('ENT_EPAY_CONFIG_LIST')" class="table-head-ground">
          <div class="table-layer">
            <jeepay-text-up :placeholder="'EPay商户PID'" v-model:value="vdata.searchData.pid" />
            <jeepay-text-up :placeholder="'关联商户号'" v-model:value="vdata.searchData.mchNo" />
      ...
      >
        <template #opRow>
          <a-button v-if="$access('ENT_EPAY_CONFIG_ADD')" type="primary" @click="addFunc" class="mg-b-30">新建</a-button>
        </template>
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'state'">
            <JeepayTableColState
              :state="record.state"
              :showSwitchType="$access('ENT_EPAY_CONFIG_EDIT')"
          ...
          <template v-if="column.key === 'op'">
            <JeepayTableColumns>
              <a-button type="link" v-if="$access('ENT_EPAY_CONFIG_VIEW')" @click="viewFunc(record.id)">详情</a-button>
              <a-button type="link" v-if="$access('ENT_EPAY_CONFIG_EDIT')" @click="editFunc(record.id)">修改</a-button>
              <a-button type="link" danger v-if="$access('ENT_EPAY_CONFIG_DEL')" @click="delFunc(record.id)">删除</a-button>
            </JeepayTableColumns>
          </template>
        </template>
      </JeepayTable>
...
const tableColumns = [
  ...
  { key: 'mchNo', title: '关联商户号', dataIndex: 'mchNo', width: '140px' },
  ...
  { key: 'createdAt', title: '创建时间', width: '170px', scopedSlots: { customRender: 'createdAtSlot' } },
...
function reqTableDataFunc(params) {
  return req.list(API_URL_EPAY_CONFIG_LIST, params)
}
...
function viewFunc(id) {
  infoAddOrEdit.value.show(id, true)
}

function editFunc(id) {
  infoAddOrEdit.value.show(id, false)
}

function delFunc(id) {
  $infoBox.confirmDanger('确认删除？', '删除后不可恢复，请谨慎操作。', () => {
    req.delById(API_URL_EPAY_CONFIG_LIST, id).then((res) => {
...
function changeState(record, newState) {
  req.updateById(API_URL_EPAY_CONFIG_LIST, record.id, { id: record.id, state: newState }).then((res) => {
