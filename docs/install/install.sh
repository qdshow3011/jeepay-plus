...
## 一键启动openhubs-pay服务，包含mysqlDB/MQ/redis/javaservice/nginx   .Power by terrfly
...
if ! [ -f "./config.sh" ]; then
    echo '下载默认配置文件。'
    wget -O config.sh https://gitee.com/jeequan/openhubs-pay/raw/master/docs/install/config.sh
fi
...
echo "[2] 拉取项目源代码文件.... "
cd $rootDir/sources
git clone https://gitee.com/jeequan/openhubs-pay.git
# cd openhubs-pay && git checkout -b dev origin/dev # 切换到dev分支。
echo "[2] Done. "

#源码中install.sh文件目录
sourcesInstallPath=$rootDir/sources/openhubs-pay/docs/install

# 创建一个 bridge网络
docker network create openhubs-pay-net
...
# 镜像启动
docker run -p 3306:3306 --name mysql8 --network=openhubs-pay-net  \
...
echo "[3] 初始化数据导入 ...... "
# 创建数据库  && 导入数据
echo "CREATE DATABASE openhubs-paydb DEFAULT CHARACTER SET utf8mb4" | docker exec -i mysql8 mysql -uroot -p$mysql_pwd
docker exec -i mysql8 sh -c "mysql -uroot -p$mysql_pwd --default-character-set=utf8mb4  openhubs-paydb" < $rootDir/sources/openhubs-pay/docs/sql/init.sql
...
# 镜像启动
docker run -p 6379:6379 --name redis6 --network=openhubs-pay-net  \
--restart=always --privileged=true \
-v /etc/localtime:/etc/localtime:ro \
...
docker run -p 8161:8161 -p 61616:61616 --name activemq5 --network=openhubs-pay-net \
--restart=always \
-v /etc/localtime:/etc/localtime:ro \
-d openhubs-pay/activemq:5.15.16
...
# 复制java配置文件
cd $rootDir/service/configs/ && cp -r $rootDir/sources/openhubs-pay/conf/* .


echo "[6.1] 下载并启动 java 项目 [ openhubs-paymanager  ] .... "
# 运行 java项目
docker run -itd --name openhubs-paymanager --restart=always --network=openhubs-pay-net \
-p 9217:9217 \
-v /etc/localtime:/etc/localtime:ro \
-v $rootDir/service/logs:/openhubs-payhomes/service/logs \
-v $rootDir/service/uploads:/openhubs-payhomes/service/uploads \
-v $rootDir/service/configs/manager/application.yml:/openhubs-payhomes/service/app/application.yml \
-d openhubs-pay/openhubs-pay-manager

echo "[6.2] 下载并启动 java 项目 [ openhubs-paymerchant  ] .... "
# 运行 java项目
docker run -itd --name openhubs-paymerchant --restart=always --network=openhubs-pay-net \
-p 9218:9218 \
-v /etc/localtime:/etc/localtime:ro \
-v $rootDir/service/logs:/openhubs-payhomes/service/logs \
-v $rootDir/service/uploads:/openhubs-payhomes/service/uploads \
-v $rootDir/service/configs/merchant/application.yml:/openhubs-payhomes/service/app/application.yml \
-d openhubs-pay/openhubs-pay-merchant

echo "[6.3] 下载并启动 java 项目 [ openhubs-paypayment  ] .... "
# 运行 java项目
docker run -itd --name openhubs-paypayment --restart=always --network=openhubs-pay-net \
-p 9216:9216 \
-v /etc/localtime:/etc/localtime:ro \
-v $rootDir/service/logs:/openhubs-payhomes/service/logs \
-v $rootDir/service/uploads:/openhubs-payhomes/service/uploads \
-v $rootDir/service/configs/payment/application.yml:/openhubs-payhomes/service/app/application.yml \
-d openhubs-pay/openhubs-pay-payment
...
wget https://gitee.com/jeequan/openhubs-pay-ui/releases/download/v1.10.0/html.tar.gz
tar -vxf html.tar.gz
...
docker logs openhubs-paypayment

echo ">>>>>>> "
echo ">>>>>>> "
echo ">>>>>>>安装完成， 所有的配置文件和项目文件都在：$rootDir 文件夹中。 "
echo ">>>>>>>项目访问地址 （注意开通端口防火墙）：   "
echo ">>>>>>>运营平台： http://外网IP:19217   账号密码： openhubs-pay/openhubs-pay123   "
